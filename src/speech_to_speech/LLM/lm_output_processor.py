...
class LMOutputProcessor(BaseHandler[LLMOut, TTSIn | PipelineEvent]):
    ...
    def process(self, lm_output: LLMOut) -> Iterator[TTSIn | PipelineEvent]:
        ...
        logger.debug("LM processor: parts=%s", transcript_for_response_log(lm_output.parts, lm_output.response))
        ...
        for part in lm_output.parts:
            ...
            if (
                ...
                continue
            logger.debug("Forwarding to TTS: %s", transcript_for_response_log(part.text, lm_output.response))
            yield TTSInput(
                text=part.text,
