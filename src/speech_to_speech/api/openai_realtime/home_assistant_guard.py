"""Fail-closed Home Assistant selector guard for trusted local clients."""
...
class HomeAssistantSelectorRejected(ValueError):
    """A complete provider response failed the guarded-selector contract."""
...
def parse_home_assistant_guard_request(
    ...
    *,
    ...
    if not isinstance(nonce, str) or _SHA256_PATTERN.fullmatch(nonce) is None:
        return None
    if not isinstance(digest, str) or _SHA256_PATTERN.fullmatch(digest) is None or digest != expected_digest:
        return None
...
def identifier_visible_words(identifier: str) -> tuple[str, ...]:
    """Return an identifier's visible words for spoken-surface rejection."""
    words: list[str] = []
    for segment in re.split(r"[^A-Za-z0-9]+", identifier):
        words.extend(
            match.group(0).casefold()
            for match in re.finditer(r"[A-Z]+(?=[A-Z][a-z]|[0-9]|$)|[A-Z]?[a-z]+|[0-9]+", segment)
        )
    return tuple(words)


class HomeAssistantGuardReadyEvent(BaseModel):
    """Acknowledgement required before a guarded client sends input."""
