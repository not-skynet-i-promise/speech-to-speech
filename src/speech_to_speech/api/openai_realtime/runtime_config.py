...
class RuntimeConfig(BaseModel):
    ...
    def fail_home_assistant_guard(self) -> None:
        """Latch selector failure before any history/client/TTS/tool sink."""
        with self.transcript_barrier_state_guard():
            self.home_assistant_guard_failed = True
            self.chat.enable_private_content_logging()
            self.chat.suspend_compaction()
