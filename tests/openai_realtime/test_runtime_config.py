...
class TestApplySessionUpdate:
    ...
    def test_create_response_false_survives_partial_update(self):
        cfg = RuntimeConfig()
        cfg.apply_session_update(
            _parse_session(audio={"input": {"turn_detection": {"type": "server_vad", "create_response": False}}})
        )
        cfg.apply_session_update(
            _parse_session(audio={"input": {"turn_detection": {"type": "server_vad", "threshold": 0.8}}})
        )
